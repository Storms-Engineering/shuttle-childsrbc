...
$class_intro = NULL;
	...
	if ( ! is_front_page() ) {
		...
		if ( empty( $shuttle_general_introswitch ) or $shuttle_general_introswitch == '1' ) {
			//Storms Edit:  Adding a featured image to a page
			$large_image_url = NULL;
			if ( has_post_thumbnail()) {
			$large_image_url = wp_get_attachment_image_src( get_post_thumbnail_id(), 'large');
			}
			//Storms Edit: Adding a picture to the 404 page
			else if(is_404())
			{
				$large_image_url = ['http://127.0.0.1/wp-content/uploads/2018/10/DSC_0358-e1540418426282.jpg'];
			}
			//Storms Edit:  Size change for header image on about page
			$size = "200px";
			if (get_the_title() == "About")
				$size = "250px";
			echo	'<div id="intro" style="height:',$size,';background-repeat: no-repeat;background-position: bottom;background-image:url(' . esc_attr($large_image_url[0]) . ');" class="' . esc_attr( $class_intro ) . '"><div class="wrap-safari"><div id="intro-core">',
					'<h1 class="page-title">',
